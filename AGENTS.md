...
The `useFilteredNavGroups()` hook in `src/hooks/use-nav.ts` passes navigation groups through unchanged. Extend it to add custom filtering logic.
...
- Check the nav config and verify the item is included in `nav-config.ts`
